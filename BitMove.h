...
class BitMove {
   ...
   public:
    BitMove(int from, int to, int piece, int promotion, int capture, int doublepush, int enpassant, int castling) {
...
class BitMoveList {
   public:
    ...
    void add_move(BitMove move) {
        moves.push_back(move);
    };
    void print();
};

void BitMoveList::print() {
    if(moves.empty()) {
    ...
    for(const BitMove& move : moves) {
        std::cout << "     " << sq_to_coord[move.get_from()] << sq_to_coord[move.get_to()]
